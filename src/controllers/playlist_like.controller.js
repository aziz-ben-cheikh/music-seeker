...
class playlist_likeController {
    ...
    async createplaylist_like(req, res) {
        try {
            const playlist_like = await playlist_likeService.createplaylist_like(req.body);
    ...
    async getplaylist_likebyid(req, res) {
        try {
            const playlist_like = await playlist_likeService.getplaylist_likebyid(req.params.id);
    ...
    async updateplaylist_like(req, res) {
        try {
            const updatedplaylist_like = await playlist_likeService.updateplaylist_like(req.params.id, req.body);
    ...
    async deleteplaylist_like(req, res) {
        try {
            const result = await playlist_likeService.deleteplaylist_like(req.params.id);
            res.json(result);
        } catch (error) {
            if (error instanceof NotFoundError) {
                res.status(404).json({ error: error.message });
